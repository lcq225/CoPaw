...
class AutoPatternDetector:
    ...
    def __init__(
        ...
        self.error_by_category: Dict[str, List[ErrorRecord]] = defaultdict(
            list,
        )
    ...
    def _detect_anomaly_patterns(self) -> None:
        ...
        for error_type, hourly_data in self.hourly_counts.items():
            ...
            # Detect anomalies
            for _, count in hourly_data.items():
    ...
    def _calculate_time_span(self, errors: List[ErrorRecord]) -> float:
        ...
        try:
            first = datetime.fromisoformat(
                errors[0].timestamp.replace("Z", "+00:00"),
            )
            last = datetime.fromisoformat(
                errors[-1].timestamp.replace("Z", "+00:00"),
            )
    ...
    def _generate_frequency_suggestion(
        ...
        if frequency > 10:
            return (
                ...
                f"Add validation or error handling."
            )
        elif frequency > 5:
            return (
    ...
    def generate_suggestions(self) -> List[Dict[str, Any]]:
        ...
        # Sort by priority
        suggestions.sort(
            key=lambda x: int(x["priority"]),  # type: ignore[call-overload]
            reverse=True,
        )
        return suggestions
        ...
        return {
            ...
            "high_priority_suggestions": len(
                [
                    ...
                    if self._calculate_priority(p) >= 7
                ],
            ),
